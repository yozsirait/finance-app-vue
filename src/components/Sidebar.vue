<template>
  ...
  >
    ...
    <div class="flex items-center justify-between p-4">
      <h1 v-if="isOpen" class="text-xl font-bold text-gray-800 dark:text-gray-100">Finance</h1>
      <button
        @click="$emit('toggle')"
        ...
        <li v-for="(item, idx) in menu" :key="idx">
          ...
          <div v-else>
            ...
            >
              <component :is="item.icon" class="w-5 h-5" />
              <span v-if="isOpen">{{ item.label }}</span>
              <ChevronDown v-if="isOpen" :class="['ml-auto transition-transform', openIndex === idx ? 'rotate-180' : '']" />
            </button>
            <ul
              v-if="isOpen && openIndex === idx"
